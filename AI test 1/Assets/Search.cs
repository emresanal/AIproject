...
public class SearchAgent{
	...
	private static Vector3 [] direc = [up,right,left,down];
	...
	public List<Vector3> bfs_search(Vector3 pos){
		...
		//while(q.Count != 0){
			...
			legals = PlayerMovements.getLegalActions(cur_pos);
	...
	public List<Vector3> a_star_search(Vector3 pos){
		...
		//while(q.Count != 0){
			...
			for(int i = 0; i < direc.length; i++){
				...
				next_state.cost =  cur_state.act_cost + 1 + heuristic(new_pos);
				next_state.act_cost = cur_state.act_cost + 1;
